...
def run_sft(
    ...
    # 创建基础 data_collator
    base_data_collator = SFTDataCollatorWith4DAttentionMask(
        template=template,
    ...
    # 安装 codec boundary hooks(如果启用)
    hook_cleanup_callback, codec_state, data_collator = maybe_attach_middle_layer_hooks(
        model, tokenizer, base_data_collator
    )
